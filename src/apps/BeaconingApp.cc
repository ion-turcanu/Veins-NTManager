...
void BeaconingApp::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        startBeaconingAt = par("startBeaconingAt").intValue();
        ...
        if (par("avoidBeaconSynchronization").boolValue() == true) {
            ...
            if (simTime() < startBeaconingAt) firstBeacon = startBeaconingAt + randomOffset;
            else firstBeacon = simTime() + randomOffset;
...
simtime_t BeaconingApp::computeAsynchronousSendingTime(simtime_t interval, ChannelType chan)
{
...
void BeaconingApp::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    wsm->setRecipientAddress(rcvId);
    wsm->setBitLength(headerLength);

    if (BeaconMessage* bsm = dynamic_cast<BeaconMessage*>(wsm)) {
...
void BeaconingApp::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    Enter_Method_Silent();
...
void BeaconingApp::handlePositionUpdate(cObject* obj)
{
...
void BeaconingApp::handleParkingUpdate(cObject* obj)
{
    isParked = mobility->getParkingState();
}

void BeaconingApp::handleLowerMsg(cMessage* msg)
{
...
void BeaconingApp::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind()) {
...
void BeaconingApp::startService(Channel channel, int serviceId, std::string serviceDescription)
{
...
void BeaconingApp::stopService()
{
    cancelEvent(sendWSAEvt);
    currentOfferedServiceId = -1;
}

void BeaconingApp::sendDown(cMessage* msg)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDown(msg);
}

void BeaconingApp::sendDelayedDown(cMessage* msg, simtime_t delay)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDelayedDown(msg, delay);
}

void BeaconingApp::checkAndTrackPacket(cMessage* msg)
{
    if (dynamic_cast<BeaconMessage*>(msg)) {
